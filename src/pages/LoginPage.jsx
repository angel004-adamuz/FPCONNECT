...
export default function LoginPage() {
  ...
  const [formData, setFormData] = useState({
    ...
    role: 'ALUMNO', recoveryQuestion: '', recoveryAnswer: '',
  });
  ...
  const [recoveryData, setRecoveryData] = useState({
    email: '', question: '', answer: '', newPassword: '', confirmPassword: '',
  });

  useEffect(() => {
    if (isAuthenticated && user) console.log('✓ Usuario autenticado:', user);
    ...
    <div className="fp-login-shell">
      <div className="fp-login-card">
        ...
        <section className="fp-login-hero" aria-label="Presentación FPConnect"
          style={{ backgroundImage: 'url(https://images.unsplash.com/photo-1522202176988-66273c2fd55f?w=1200&q=80)', backgroundSize: 'cover', backgroundPosition: 'center', position: 'relative', overflow: 'hidden' }}>
          <div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(135deg, rgba(0,15,10,0.88) 0%, rgba(0,35,25,0.80) 60%, rgba(0,20,40,0.75) 100%)' }} />
          <div style={{ position: 'relative', zIndex: 1 }}>
            <div className="fp-login-kicker">Red FP para estudiar, conectar y encontrar oportunidades</div>
            <h1 className="fp-login-title">Tu red profesional empieza antes del primer contrato</h1>
            <p className="fp-login-copy">Explora centros, descubre empresas, comparte proyectos y crea una red que acompañe tu etapa de FP sin parecer una plataforma aburrida.</p>
            <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 28 }}>
              <div style={{ display: 'flex' }}>
                {['Ana M', 'Luis G', 'Sara P', 'Juan R', 'María T'].map((name, i) => (
                  <img key={name} src={`https://ui-avatars.com/api/?name=${encodeURIComponent(name)}&background=${['00A878','2563EB','7C3AED','EC4899','F59E0B'][i]}&color=fff&size=40&bold=true`} alt={name}
            ...
            <div className="fp-login-metrics" aria-label="Ventajas principales">
              <div className="fp-login-metric"><strong>3 roles</strong><span>Alumnos, centros y empresas en el mismo espacio.</span></div>
              <div className="fp-login-metric"><strong>Feed vivo</strong><span>Publicaciones, comentarios y perfiles para explorar.</span></div>
              <div className="fp-login-metric"><strong>FP real</strong><span>Centros, prácticas, noticias y talento técnico.</span></div>
            </div>
          </div>
        </section>
        ...
        <section className="fp-auth-panel" aria-label="Acceso a FPConnect">
          ...
          {!isRecoveryMode && (
            <button onClick={handleGoogleLogin} style={{
              ...
              color: '#1f1f1f', fontSize: 14, fontWeight: 600, cursor: 'pointer',
          ...
          {!isRecoveryMode && (
            <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 16 }}>
              ...
              <span style={{ color: '#ffffff66', fontSize: 12 }}>o continúa con email</span>
              <div style={{ flex: 1, height: 1, background: 'rgba(255,255,255,0.12)' }} />
            </div>
          )}
